'use client'
import { CheckboxGroup as CCheckBoxGroup, Checkbox, CheckboxGroupProps, Stack, Text } from "@chakra-ui/react";
...
const allValue = "Все";
const CheckBoxGroup: FC<ICheckBoxProps> = ({ labels, value, paramName, withAll=true, setValue }) => {
        ...
        <CCheckBoxGroup onChange={handleSetValue} value={value}>
            <Stack spacing={[1, 5]} direction={['column', 'column']}>
                {labels.map(item => {
                    ...
                    const disabled = !exeptDisable && !(Object.keys(tags).includes(paramName) && Object.values(tags).flat().map(t=>t.toLowerCase()).includes(item.toLowerCase()))
                    return (
