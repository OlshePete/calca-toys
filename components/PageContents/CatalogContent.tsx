...
import { IAllProductsContent, IProduct, IResponseData, TypeLabel } from "@/types/api";
...
const filterByTypeParams = (params:Record<string, string[]> | null, type: string = 'page'):Record<string, string[]> | null => {
...
const filterByPrice = (products:IResponseData<IProduct>[],price:[number,number]) => [...products].filter(product=>{
...
const CatalogContent: FC<ICatalogContentProps> = ({label, categoriesName, products, pagination, materials, priceLimits }) => {
   ...
   console.log('products %%%%',products);
    ...
    const [filteredProducts, setFilteredProducts] = useState<IResponseData<IProduct>[]>(filterByPrice(products.data,price));
    ...
    const handleSetSpecial:(value:string[])=>void = (value)=>{
        setSpecials(()=>{
                ...
                value
                    .forEach(label=>{
                        const param = Object.entries(SPECIAL_LIST).find(([key,special])=>special===label.toLowerCase())?.[0]
    ...
    return (
        <>
            <CustomTitle variant={"post_header"} pt={"60px"} pb={"40px"}>{label}</CustomTitle>
                    ...
                    <>
                        <Accordion allowMultiple>
                            {[defaultCategory,...categoriesName].map(({ title, paramName, variants }) => (
                                <AccordionItem
                                    key={paramName}
                                    propValue={params ? params?.[paramName] ? params[paramName].map(n => n.toLowerCase()) : [] : []}
                                    label={title}
                                    paramName={paramName}
                        ...
                        <CheckBoxGroup
                            ...
                            value={specials ? specials.map(special=>SPECIAL_LIST[special as keyof typeof SPECIAL_LIST]):[]}
                            paramName={"specials"}
                        />
                        <PricePicker
                            limit={priceLimits}
