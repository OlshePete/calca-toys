'use client'
import { Box, Button, Grid, GridItem, Heading, HStack, List, ListItem, Text, useBreakpointValue, VStack } from "@chakra-ui/react";
...
const BasketContent: FC<IBasketContentProps> = ({ }) => {
    ...
    const connectedProducts = Object.values(basket.items).reduce<IResponseData<IProduct>[]>((acc, item) => {
    ...
    return (
        <>
            ...
            {connectedProducts.length > 0 && <ProductsCarousel label="С этим товаром покупают" products={connectedProducts} withButton={false} mt={'44px'}/>}
        </>
    );
};
