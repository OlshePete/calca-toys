  ...
  return (
    <div>
      <div className='section fullH' style={{ background: "#FEF7E6", padding: '44px 0' }}>
        <ContentContainer>
          <Breadcrumbs />
          <CustomTitle variant={"post_header"} pt={"60px"} pb={"40px"} className="seo-title">
           {title}
          </CustomTitle>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}> 
              <CustomText variant={'post_text'}>{subtitle}</CustomText>
              <CustomText style={{lineHeight: '24px',fontSize:'16px'}} variant={'post_text'}>{description}</CustomText>
            </div> 
          ...
          <div style={{
            ...
            {variants && [...variants].map((variant, Skey) => {
              const id = variant.id
                ...
                <ListItemWrapper key={id}>
                  {baseImage && (
                    <Image
                      ...
                      src={baseImage ? `${API_URL}/cms${baseImage.attributes.formats?.medium?.url ?? baseImage.attributes.url}` : "/"}
                      fill={true}
                      style={{
                        objectFit: "cover"
                      }}
                  ...
                  >
                    <CustomTitle variant={"post_header"} color={'white'}>{title}</CustomTitle>
