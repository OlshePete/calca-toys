...
import { Box, Input, FormLabel, FormControl, Button, Textarea, Text, Checkbox, HStack, Heading, RadioGroup, Stack, Radio, useToast } from '@chakra-ui/react';
...
const CustomerFormSchema = Yup.object().shape({
  ...
  paymentFormat: Yup.string().oneOf(['card','cash'], 'Необходимо выбрать форму оплаты').required('Необходимо выбрать форму оплаты'),
});
...
const CustomerForm:FC<{setSubmitRef:(ref:HTMLButtonElement)=>void}> = ({setSubmitRef}) => {
  ...
  useEffect(()=>{
    if(submitBtn && submitBtn.current && setSubmitRef && typeof setSubmitRef === 'function') setSubmitRef(submitBtn.current)
  },[submitBtn])
    ...
    try {
      ...
      const orderResponse = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          items:Object.values(basket.items).map((item, index)=>{
      ...
      toast({
        title: 'Ошибка',
        description: (error as Error).message || 'Произошла ошибка при оформлении заказа, попробуйте заново!',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
        ...
        {({ isSubmitting,values, errors, setFieldValue, setFieldError, setFieldTouched }) => {
          ...
          <HStack>
            ...
            <FormControl >
              ...
              <ErrorMessage  className="feedback_form_label error" name="consent" component={Text}  />
            </FormControl>
            ...
            <FormControl>
              ...
              >
                ...
                }}>Наличные/картой при получении</Checkbox>
                </Stack>
              </RadioGroup>
              <ErrorMessage  className="feedback_form_label error" name="paymentFormat" component={Text}  />
            </FormControl>
            <Button
            hidden
              ref={submitBtn}
