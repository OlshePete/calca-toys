...
import { Box, Button, Icon, Image, Popover, PopoverBody, PopoverContent, VStack } from '@chakra-ui/react'
...
const HeaderMenuPopover: FC<IProps> = ({  categoryNames  }) => {
  ...
  const currentCategories = activeCategory && categoryNames?.data?.filter((c, i) =>c.attributes.type===activeCategory)
 ...
 const onClose = () => setActiveCategory(null)
  return (
    <Popover isOpen={Boolean(activeCategory)} onClose={onClose} autoFocus>
            ...
            >
              {
                currentCategories && currentCategories[currentIndex] && currentCategories[currentIndex]?.attributes.tags.data.map((variant, variantIndex) => {
                  ...
                  return <Button
                    ...
                    className='only_text'
                    onClick={()=>{
                      router.push(`/catalog/${activeCategory}?holiday=${variant.attributes.title}`)
                      onClose()
                    }}
                  >
