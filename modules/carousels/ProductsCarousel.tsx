...
function ProductsCarousel({ label, products: baseProducts, dinamicMarginLeft = true, link, withButton = true, ...boxProps }: ProductsCarouselProps) {
  const router = useRouter();
  const containerRef = useRef<HTMLDivElement>(null);
  const products = baseProducts
  const [marginLeft, setMarginLeft] = useState<string | number | undefined>('0px')
  ...
  return (
    ...
    >
        ...
        <Box display={"flex"} justifyContent={"space-between"} height={'auto'}>
          ...
          <ButtonGroup variant="outline" spacing="6"
            ...
            <IconButton
              ...
              _hover={{ svg: { circle: { fill: "#F49AA5" }, path: { fill: "#FEF7E6" }, } }}
              style={{
              }}
              icon={
      ...
      >
        <Button onClick={() => router.push(`/catalog${link}`)} variant="outline" className="text">
          смотреть все
        </Button>
      </Box>}
    </Box>
