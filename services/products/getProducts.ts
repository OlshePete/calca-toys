...
const headers = {
 ...
 const getAllProducts = async () => {
    const response = await fetch(`${API_URL}/cms/api/tovars?populate[variant][populate][image][fields]&populate[tags][populate][category]=*&populate[connected][populate][variant][populate][image][fields]=*&populate[materials][populate]=*&populate[type]=*`, {
        headers,
        next:{
            revalidate:60,
        }
  ...
  const getProductById = async (id:number|string) => {
     const response = await fetch(`${API_URL}/cms/api/tovars/${id}?populate[variant][populate][image][fields]&populate[tags][populate][category]=*&populate[connected][populate][variant][populate][image][fields]=*&populate[type]=*`, {
         headers,
         next:{
             revalidate:60,
         }
