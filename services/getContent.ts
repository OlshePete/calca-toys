import { IContactsResponse, INewsResponse, IPrivacyResponse, IStartPageContent, IResponseItemWithDataMeta, IResponseData, INewsItem, IWarrantyResponse, IPaymentResponse } from "@/types/api";
...
export const getContacts = cache(async () => {
    const response = await fetch(`${API_URL}/cms/api/contact?populate[socials][populate][icon][fields]=url&populate[essential]populate=*&populate=*`, {
        headers,
        next: {
            revalidate: 3600, // кэшируем на 1 час
        }
...
export const getNews = cache(async () => {
    const response = await fetch(`${API_URL}/cms/api/blogs?populate[cover][fields]=url&populate=*`, {
        headers,
        next: {
            revalidate: 3600, // кэшируем на 1 час
        }
...
export const getPrivacy = cache(async () => {
    const response = await fetch(`${API_URL}/cms/api/privacy?populate[paragraphs][populate]=paragraph`, {
        headers,
        next: {
            revalidate: 3600, // кэшируем на 1 час
        }
...
export const getWarranty = cache(async () => {
    const response = await fetch(`${API_URL}/cms/api/warranty?populate[variant][populate]=*&populate=*`, {
        headers,
        next: {
            revalidate: 3600, // кэшируем на 1 час
        }
...
export const getNewsById = cache(async (id: string) => {
    const response = await fetch(`${API_URL}/cms/api/blogs/${id}?populate[cover][fields]=url&populate=*`, {
        headers,
        next: {
            revalidate: 3600, // кэшируем на 1 час
        }
