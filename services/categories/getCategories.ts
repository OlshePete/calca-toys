import { IAllCategoryNames, TProductType } from '@apptypes/api';
...
const getAllCategoryName = async () => {
  const response = await fetch(
    `${API_URL}/cms/api/categories?populate[icon][fields][0]=url&populate[tags]=*`,
    {
      headers,
      next: {
        revalidate: 60,
...
const getCategoryNameByType = async (type: TProductType) => {
  const response = await fetch(
    `${API_URL}/cms/api/categories?filters[type][$eq]=${type}&populate[icon][fields][0]=url&populate[tags]=*`,
    {
      headers,
      next: {
        revalidate: 60,
  ...
  const data: IAllCategoryNames = await response.json();
  console.log(
    `Список категорий (${type}) успешно получен! Добавлено категорий: `,
    data.data.length
  );
